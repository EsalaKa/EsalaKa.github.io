...
if(page==='function-concept.html'){
 put('4. Different inputs',plot({xmin:-3,xmax:3,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:-2,y:4},{x:0,y:0},{x:2,y:4}],labels:[{x:-2,y:4,t:'(−2,4)',dx:-60,dy:-10},{x:2,y:4,t:'(2,4)',dx:10,dy:-10}]}));
}
...
if(page==='domain-range.html'){
 put('2. Range',plot({xmin:-4,xmax:4,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:0,y:0}],labels:[{x:2.2,y:6.5,t:'y=x²'}]}));
 put('4. Division',plot({xmin:-5,xmax:5,ymin:-5,ymax:5,curves:[{a:-5,b:-.15,f:x=>1/x},{a:.15,b:5,f:x=>1/x}],guides:[{x1:0,y1:-5,x2:0,y2:5}],labels:[{x:2.1,y:1.1,t:'y=1/x'}]}));
 put('5. Square roots',plot({xmin:-2,xmax:8,ymin:-1,ymax:4,curves:[{a:0,b:8,f:x=>Math.sqrt(x)}],points:[{x:0,y:0}],labels:[{x:3.8,y:2.4,t:'y=√x'}]}));
}
if(page==='linear-functions.html'){
 ...
 put('3. The intercept',plot({xmin:-3,xmax:4,ymin:-5,ymax:10,curves:[{a:-3,b:4,f:x=>2*x+1}],points:[{x:0,y:1}],labels:[{x:.1,y:1,t:'(0,1)',dx:8,dy:-8}]}));
}
if(page==='quadratic-functions.html'){
 put('1. The basic parabola',plot({xmin:-3,xmax:3,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:-2,y:4},{x:0,y:0},{x:2,y:4}],labels:[{x:-2,y:4,t:'(−2,4)',dx:-60,dy:-10},{x:2,y:4,t:'(2,4)',dx:10,dy:-10}]}));
 put('2. The sign',plot({xmin:-4,xmax:4,ymin:-5,ymax:5,curves:[{a:-4,b:4,f:x=>x*x-3},{a:-4,b:4,f:x=>-x*x+3,color:C.rust}],points:[{x:0,y:-3},{x:0,y:3}],labels:[{x:1.5,y:4,t:'a<0'},{x:1.5,y:-3.5,t:'a>0'}]}));
 put('3. The turning point',plot({xmin:-3,xmax:3,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:0,y:0}],guides:[{x1:0,y1:-1,x2:0,y2:9}]}));
 put('4. Roots',plot({xmin:0,xmax:5,ymin:-1,ymax:6,curves:[{a:0,b:5,f:x=>(x-2)*(x-3)}],points:[{x:2,y:0},{x:3,y:0},{x:2.5,y:-.25}],labels:[{x:2,y:0,t:'(2,0)',dx:-48,dy:24},{x:3,y:0,t:'(3,0)',dx:10,dy:24},{x:2.5,y:-.25,t:'vertex',dx:10,dy:18}]}));
 put('5. Completing the square',plot({xmin:-1,xmax:5,ymin:0,ymax:12,curves:[{a:-1,b:5,f:x=>(x-2)*(x-2)+3}],points:[{x:2,y:3}],guides:[{x1:2,y1:0,x2:2,y2:12}],labels:[{x:2,y:3,t:'(2,3)',dx:10,dy:-8}]}));
 put('9. Changing',plot({xmin:-3,xmax:3,ymin:-1,ymax:10,curves:[{a:-2.2,b:2.2,f:x=>2*x*x},{a:-3,b:3,f:x=>.5*x*x,color:C.rust}],labels:[{x:1.3,y:6.5,t:'2x²'},{x:2,y:2.2,t:'0.5x²'}]}));
 put('10. Why the slope',plot({xmin:-3,xmax:3,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:-1.5,y:2.25},{x:0,y:0},{x:1.5,y:2.25}],guides:[{x1:-2.2,y1:4.35,x2:-.8,y2:.15,dash:false},{x1:-.8,y1:0,x2:.8,y2:0,dash:false},{x1:.8,y1:.15,x2:2.2,y2:4.35,dash:false}]}));
}
if(page==='polynomial-functions.html'){
 put('3. The constant term',plot({xmin:-3,xmax:3,ymin:-6,ymax:8,curves:[{a:-3,b:3,f:x=>x*x*x-3*x+2}],points:[{x:0,y:2}],labels:[{x:.1,y:2,t:'(0,2)',dx:8,dy:-8}]}));
 put('4. Roots',plot({xmin:-1,xmax:6,ymin:-3,ymax:7,curves:[{a:-1,b:6,f:x=>(x-2)*(x-3)}],points:[{x:2,y:0},{x:3,y:0}],labels:[{x:2,y:0,t:'(2,0)',dx:-48,dy:24},{x:3,y:0,t:'(3,0)',dx:10,dy:24}]}));
 put('6. Odd and even',plot({xmin:-2.5,xmax:2.5,ymin:-8,ymax:8,curves:[{a:-2,b:2,f:x=>x*x*x},{a:-1.65,b:1.65,f:x=>x*x*x*x-2,color:C.rust}]}));
 ...
 put('9. Repeated roots',plot({xmin:-2,xmax:4,ymin:-3,ymax:6,curves:[{a:-2,b:4,f:x=>(x+1)*(x-1)*(x-1)}],points:[{x:-1,y:0},{x:1,y:0}],labels:[{x:-1,y:0,t:'crosses',dx:-55,dy:24},{x:1,y:0,t:'touches',dx:10,dy:24}]}));
}
if(page==='rational-functions.html'){
 put('1. The reciprocal',plot({xmin:-5,xmax:5,ymin:-5,ymax:5,curves:[{a:-5,b:-.15,f:x=>1/x},{a:.15,b:5,f:x=>1/x}],guides:[{x1:0,y1:-5,x2:0,y2:5}],labels:[{x:2.1,y:1.1,t:'y=1/x'}]}));
