(() => {
  ...
  const box=()=>{const d=document.createElement('div');d.className='math-graph';return d;};

  function plot({xmin,xmax,ymin,ymax,curves=[],guides=[],points=[],openPoints=[],labels=[],equal=false}){
    ...
    const defs=el('defs'),cid='clip-'+Math.random().toString(36).slice(2),clip=el('clipPath',{id:cid});clip.appendChild(el('rect',{x:m,y:m,width:pw,height:ph}));defs.appendChild(clip);svg.appendChild(defs);
    ...
    curves.forEach((c,j)=>{
      ...
      for(let i=0;i<=n;i++){
        ...
        if(!Number.isFinite(y)||y<ymin-0.2*(ymax-ymin)||y>ymax+0.2*(ymax-ymin)){if(pts.length>1)g.appendChild(el('polyline',{points:pts.join(' '),fill:'none',stroke:c.color||[C.blue,C.rust,C.green][j%3],'stroke-width':c.width||3,'stroke-linejoin':'round','stroke-linecap':'round'}));pts=[];prev=null;continue;}
        if(prev!==null&&Math.abs(y-prev)>0.45*(ymax-ymin)){if(pts.length>1)g.appendChild(el('polyline',{points:pts.join(' '),fill:'none',stroke:c.color||[C.blue,C.rust,C.green][j%3],'stroke-width':c.width||3}));pts=[];}
        pts.push(`${X(x).toFixed(2)},${Y(y).toFixed(2)}`);prev=y;
      }
      if(pts.length>1)g.appendChild(el('polyline',{points:pts.join(' '),fill:'none',stroke:c.color||[C.blue,C.rust,C.green][j%3],'stroke-width':c.width||3,'stroke-linejoin':'round','stroke-linecap':'round'}));
    });
    svg.appendChild(g);
    ...
    const d=box();d.appendChild(svg);return d;
  }
  ...
  function put(title,node){
    ...
    let p=h.nextElementSibling,last=h;while(p&&p.tagName!=='H3'&&!p.classList.contains('lesson-navigation')){last=p;if(p.tagName==='P')break;p=p.nextElementSibling;}last.insertAdjacentElement('afterend',node);
  }

  if(page==='function-concept.html'){
    put('4. Different inputs',plot({xmin:-3,xmax:3,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:-2,y:4},{x:0,y:0},{x:2,y:4}],labels:[{x:-2,y:4,t:'(−2,4)',dx:-65,dy:-10},{x:2,y:4,t:'(2,4)',dx:10,dy:-10},{x:2.2,y:7,t:'y=x²'}]}));
    put('6. The graph',plot({xmin:-2,xmax:4,ymin:-2,ymax:10,curves:[{a:-2,b:4,f:x=>2*x+1}],points:[{x:2,y:5}],guides:[{x1:2,y1:0,x2:2,y2:5},{x1:0,y1:5,x2:2,y2:5}],labels:[{x:2,y:5,t:'(2,f(2))',dx:10,dy:-10}]}));
  }
  ...
  if(page==='domain-range.html'){
    ...
    put('4. Division',plot({xmin:-5,xmax:5,ymin:-5,ymax:5,curves:[{a:-5,b:-.12,f:x=>1/x},{a:.12,b:5,f:x=>1/x}],guides:[{x1:0,y1:-5,x2:0,y2:5},{x1:-5,y1:0,x2:5,y2:0}],labels:[{x:.25,y:4.2,t:'x=0 excluded'},{x:2.1,y:1.1,t:'y=1/x'}]}));
  ...
  if(page==='linear-functions.html'){
    ...
    put('3. The intercept',plot({xmin:-3,xmax:4,ymin:-4,ymax:9,curves:[{a:-3,b:4,f:x=>2*x+1}],points:[{x:0,y:1}],labels:[{x:.1,y:1,t:'(0,1)',dx:8,dy:-8}]}));
    put('4. Positive',plot({xmin:-4,xmax:4,ymin:-4,ymax:4,curves:[{a:-4,b:4,f:x=>.75*x+1},{a:-4,b:4,f:x=>-.75*x+1,color:C.rust},{a:-4,b:4,f:x=>-2,color:C.green}],labels:[{x:2.2,y:3,t:'m>0'},{x:2.2,y:-.8,t:'m<0'},{x:2.2,y:-1.7,t:'m=0'}]}));
  }
  if(page==='quadratic-functions.html'){
    ...
    put('2. The sign',plot({xmin:-4,xmax:4,ymin:-5,ymax:5,curves:[{a:-3,b:.8,f:x=>(x+1.5)**2-3},{a:-.5,b:4,f:x=>-(x-1.5)**2+3,color:C.rust}],points:[{x:-1.5,y:-3},{x:1.5,y:3}],labels:[{x:-2.8,y:3.8,t:'a>0'},{x:2.2,y:3.8,t:'a<0'}]}));
    ...
    put('5. Completing the square',plot({xmin:-1,xmax:5,ymin:0,ymax:12,curves:[{a:-1,b:5,f:x=>(x-2)**2+3}],points:[{x:2,y:3}],guides:[{x1:2,y1:0,x2:2,y2:12}],labels:[{x:2,y:3,t:'(2,3)',dx:10,dy:-10}]}));
    ...
    put('10. Why the slope',plot({xmin:-3,xmax:3,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>x*x}],points:[{x:-1.5,y:2.25},{x:0,y:0},{x:1.5,y:2.25}],guides:[{x1:-2.3,y1:4.65,x2:-.7,y2:-.15,dash:false},{x1:-.8,y1:0,x2:.8,y2:0,dash:false},{x1:.7,y1:-.15,x2:2.3,y2:4.65,dash:false}],labels:[{x:-2.7,y:6,t:'negative slope'},{x:.2,y:.6,t:'slope 0'},{x:1.5,y:6,t:'positive slope'}]}));
  }
  if(page==='polynomial-functions.html'){
    put('3. The constant term',plot({xmin:-3,xmax:4,ymin:-6,ymax:8,curves:[{a:-3,b:4,f:x=>.25*x**3-x+2}],points:[{x:0,y:2}],labels:[{x:.1,y:2,t:'(0,a₀)',dx:8,dy:-8}]}));
    ...
    put('6. Odd and even',plot({xmin:-3,xmax:3,ymin:-10,ymax:10,curves:[{a:-2.15,b:2.15,f:x=>x**3},{a:-1.75,b:1.75,f:x=>x**4-4,color:C.rust}],labels:[{x:1.25,y:5,t:'odd degree'},{x:-1.7,y:5,t:'even degree'}]}));
    put('7. Cubic',plot({xmin:-3,xmax:3,ymin:-5,ymax:5,curves:[{a:-3,b:3,f:x=>x**3-3*x}],points:[{x:-1,y:2},{x:1,y:-2}],labels:[{x:-1,y:2,t:'local max',dx:-75,dy:-10},{x:1,y:-2,t:'local min',dx:10,dy:20}]}));
  }
  if(page==='rational-functions.html'){
    put('1. The reciprocal',plot({xmin:-5,xmax:5,ymin:-5,ymax:5,curves:[{a:-5,b:-.12,f:x=>1/x},{a:.12,b:5,f:x=>1/x}],guides:[{x1:0,y1:-5,x2:0,y2:5},{x1:-5,y1:0,x2:5,y2:0}],labels:[{x:2.1,y:1.1,t:'y=1/x'}]}));
    ...
    put('6. A removable',plot({xmin:-2,xmax:4,ymin:-1,ymax:6,curves:[{a:-2,b:.98,f:x=>x+1},{a:1.02,b:4,f:x=>x+1}],openPoints:[{x:1,y:2}],labels:[{x:1,y:2,t:'hole (1,2)',dx:12,dy:-12},{x:2.3,y:3.7,t:'y=x+1'}]}));
  }
  if(page==='exponential-functions.html'){
    put('3. Exponential decay',plot({xmin:-3,xmax:4,ymin:-1,ymax:9,curves:[{a:-3,b:3,f:x=>2**x},{a:-3,b:3,f:x=>.5**x,color:C.rust}],points:[{x:0,y:1}],labels:[{x:1.6,y:4.5,t:'2ˣ'},{x:-2.6,y:4.5,t:'(1/2)ˣ'}]}));
    put('10. Linear versus exponential',plot({xmin:0,xmax:6,ymin:0,ymax:35,curves:[{a:0,b:6,f:x=>4*x+1},{a:0,b:5,f:x=>2**x,color:C.rust}],labels:[{x:4.5,y:19,t:'linear'},{x:4.1,y:25,t:'exponential'}]}));
  }
